...
4. **The node proves the word was generated fairly.** It publishes the word with a cryptographic proof and a signature from its enclave. Those artifacts are in the fulfillment transaction, the router commits to them on-chain, and anyone can re-check them.
...
A third check ties both to the chain: the router stores `keccak256(proofCommitment ‖ fulfilledAt)` for every fulfillment it accepts and deletes it if a challenge proves the proof wrong, so `fulfillmentCommitted` confirms that _this_ proof is the one the router accepted and that it still stands.
...
```ts
{
  ...
  requests: Array<RandomnessRequestV1 & {
    artifacts?: {            // from the router's RandomnessFulfilled log, for independent re-verification
      ...
      uPoint?: [HexString, HexString];                            // fast-verify hint U = s·G − c·Y (derived)
      vComponents?: [HexString, HexString, HexString, HexString]; // fast-verify hints [s·H, c·Γ] (derived)
    };
    request?: {                            // the router request echoed in the fulfillment calldata
      requester: HexString;                //   (the casino diamond)
      sequence: string;
      fulfiller: HexString;                //   the enclave the router assigned at request time
      assignedAt: string;
      payment: string;
      gasPrice: string;
      clientData: HexString;               //   the committed casino session snapshot
    };
    fulfilledAt?: number;                  // block timestamp of the fulfillment (seconds)
    ...
    checks?: {
      ...
      fastVerifyComponentsMatch: boolean;  // fast-verify hints derivable from (publicKey, proof, alpha)
      enclaveSignatureValid: boolean;      // EIP-712 signature recovers
      signerMatchesFulfiller: boolean;     // signer == assigned enclave
      fulfillmentCommitted: boolean;       // router still stores keccak(proofCommitment ‖ fulfilledAt)
    };
    valid?: boolean;                       // all six checks passed
  }>;
}
```

The router keeps no per-request state: the assigned request is hashed into `requestId`, echoed back
by the node in the fulfillment calldata, and only `keccak256(proofCommitment ‖ fulfilledAt)` is
stored afterwards. The host therefore reads the fulfillment transaction — the `RandomnessFulfilled`
log for `randomness`, `proof` and `enclaveSignature`, the calldata for the echoed `request` (accepted
only when it hashes back to `requestId`, which is what makes its `fulfiller` trustworthy) — plus two
router views: the node's registered public key (checked to actually hash to the node's address) and
the stored commitment. `uPoint` / `vComponents` are the `ECVRFVerifier.fastVerify` hints, derived in
the browser from the public key, the proof and `alpha`; they are pure functions of those public
inputs, so anyone can recompute them (and feed them to the on-chain verifier) without trusting the host.
...
The host verifies client-side (in the user's browser) against on-chain data: it reads the fulfillment transaction and the node's registered public key from the Verify Network router and runs the ECVRF + EIP-712 + commitment checks locally. The `artifacts`, `request`, `fulfilledAt`, `fulfiller`, `nodePublicKey`, and `routerAddress` fields are returned precisely so anyone can repeat the verification without trusting the host's verdict — the inputs are all public chain data keyed by `requestId`, and `request` + `proof` + `fulfilledAt` are exactly the arguments of the router's permissionless `challengeFulfillment`, should a proof ever fail to verify.
